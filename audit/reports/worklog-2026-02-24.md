...
### Z. Phase 1 continued: shared component lint burn-down

- Continued warning cleanup on shared reusable components and table primitives:
  - `client/src/components/ExportButton.vue`
  - `client/src/components/TenantSelector.vue`
  - `client/src/components/ui/table/BaseTableActions.vue`
  - `client/src/components/ui/table/BaseTableCell.vue`
  - `client/src/components/ui/table/BaseTableHead.vue`
  - `client/src/components/ui/table/BaseTableRow.vue`
- Cleanup type: remove unused `props` bindings while preserving prop contracts.
- Focused lint scan result improved:
  - before: `28 warnings, 0 errors`
  - after: `22 warnings, 0 errors`
- Sanity lint on modified files: pass (no errors/warnings).

### AA. Phase 3/4/5/6/7 execution closure (no deploy step in this batch)

- Phase 3 (quality burn-down) finalized for audited scope:
  - cleaned remaining unused-symbol warnings in shared components (`CustomerBulkMessageModal`, `ImageCropperModal`, `MidtransPaymentModal`, `NotificationDropdown`, `ReceiptPrinter`, `ReceiptTemplateManager`, `ReportExportModal`, `SubscriptionReceiptTemplateManager`, `ToastContainer`, `Tooltip`, `WelcomeTour`, `KitchenSettingsModal`, `BaseInput`).
  - focused lint command over audited modules/components now returns `0 warnings, 0 errors`.
- Phase 4 (deep UI journeys) remains green with expanded coverage from prior batch:
  - customers UI `4/4`, orders UI `4/4`, tenant detail UI `3/3`.
- Phase 5 (payment realistic contract) strengthened:
  - `client/cypress/e2e/payment-webhook-api.cy.ts` now includes a correctly signed callback payload path using SHA-512 signature generation.
  - live result: `5/5 passing`.
- Phase 6 (CI gate) implemented:
  - added `.github/workflows/quick-release-gate.yml`
  - gates: health identity check + API sanity trio + UI sanity trio (secret-backed).
  - added root script `npm run verify:health:identity` via `scripts/verify-health-identity.js`.
- Phase 7 (verification + closure docs, pre-deploy batch):
  - `npm run verify:health:identity` pass (`appCommitSha=b453257...`, `commitSource=SOURCE_COMMIT`, DB connected).
  - payment webhook signed contract test pass (`5/5`).

### AB. Continued pre-deploy hardening batch (no deploy)

- Ran full frontend lint again and continued warning burn-down outside the audited core pages:
  - removed additional unused bindings/imports in composables/utilities/router/test scaffolding (`useAutoSave`, `useCache`, `useFocusTrap`, `usePendingOrders`, `useSearchSuggestions`, `useShiftReminder`, `useTenantCheck`, `main.ts`, `router/index.ts`, `utils/*`, `phase29-e2e.cy.ts`).
  - global lint result improved from `131` to `112` warnings (`0 errors`).
- Expanded payment callback UI coverage with malformed query safety check:
  - `client/cypress/e2e/payment-callback-ui.cy.ts` now `4/4 passing`.
- Reconfirmed payment webhook contract and observability gate after updates:
  - `payment-webhook-api.cy.ts`: `5/5 passing`.
  - `verify:health:identity`: pass (`appCommitSha=befb419...`, `commitSource=SOURCE_COMMIT`, DB connected).
