<template>
  <div
    class="flex flex-col gap-6 font-display p-6 lg:p-8 bg-slate-50 min-h-screen"
  >
    ...
    >
      <div
        class="bg-white dark:bg-slate-800 rounded-full shadow-lg px-4 py-2 flex items-center gap-2 border border-slate-200 dark:border-slate-700"
      >
        <div
          v-if="isRefreshing"
          class="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"
        ></div>
        <span v-else class="material-symbols-outlined text-blue-500"
          >arrow_downward</span
        >
    ...
    >
      <div class="flex flex-col gap-1">
        <h1
          class="text-3xl font-bold text-slate-900 tracking-tight leading-tight"
        >
          Orders & Transactions
        </h1>
      ...
      <div class="flex gap-3">
        ...
        <div class="relative group">
          <button
            ...
            class="px-3 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition flex items-center gap-2 text-sm font-bold shadow-sm"
          >
          ...
          <div
            v-if="showColumnMenu"
            class="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 p-3 z-50 animate-in fade-in zoom-in-95 duration-200"
          >
            <div class="space-y-2">
              <label
              ...
              >
                <input
                  type="checkbox"
                  :checked="isColumnVisible('orderInfo')"
                  @change="toggleColumn('orderInfo')"
              ...
              >
                <input
                  type="checkbox"
                  :checked="isColumnVisible('customer')"
                  @change="toggleColumn('customer')"
                  class="rounded text-blue-500 focus:ring-blue-500"
                />
              ...
              >
                <input
                  type="checkbox"
                  :checked="isColumnVisible('total')"
                  @change="toggleColumn('total')"
                  class="rounded text-blue-500 focus:ring-blue-500"
                />
    ...
    <!-- Filters Bar -->
    <div
      class="bg-white rounded-2xl border border-slate-100 shadow-sm p-2 flex flex-col xl:flex-row gap-2"
    >
      <!-- Search -->
    ...
    <div
      ...
      class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200 animate-scale-in"
    >
      <div
        class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-4"
      >
        <span class="material-symbols-outlined text-[40px] text-slate-300"
          >shopping_bag</span
        >
      </div>
      <h3 class="text-xl font-bold text-slate-900 mb-2">Belum Ada Pesanan</h3>
      <p class="text-slate-500 text-center max-w-md mb-6">
        Pesanan yang masuk atau transaksi baru akan muncul di sini.
      </p>

      <router-link
        to="/pos"
        class="px-6 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-bold transition-all shadow-lg shadow-blue-500/20 flex items-center gap-2"
      >
    ...
    <div v-else class="space-y-6">
        ...
        >
          ...
          <div class="flex justify-between items-start mb-4">
            <div class="flex items-center gap-3">
              ...
              >
                <span class="material-symbols-outlined text-[20px]">{{
                  isOrderSelected(order.id) ? "check_circle" : "circle"
                }}</span>
              </div>
              <div>
            ...
            >
              ...
              >
                <option value="PENDING" class="bg-white text-slate-900">
                  Pending
                </option>
                <option value="PROCESSING" class="bg-white text-slate-900">
                  Processing
                </option>
                <option value="COMPLETED" class="bg-white text-slate-900">
                  Selesai
                </option>
                <option value="CANCELLED" class="bg-white text-slate-900">
                  Batalkan
                </option>
              </select>
              <span
                class="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-[10px] opacity-50 group-hover/status:opacity-100 transition-opacity"
                >▼</span
              >
            </div>
            <span
          ...
          <div class="grid grid-cols-2 gap-2">
            <button
              ...
              class="py-2.5 px-4 rounded-xl bg-slate-100 text-slate-600 font-bold text-xs hover:bg-slate-200 transition"
            >
              Detail
            </button>
            <button
              @click="printReceipt(order)"
              class="py-2.5 px-4 rounded-xl bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 font-bold text-xs hover:bg-blue-100 transition"
            >
              Print
            </button>
      ...
      <div
        ...
        class="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 animate-in slide-in-from-bottom-4 duration-200"
      >
        <div class="flex items-center gap-3 pr-6 border-r border-slate-700">
          <div
            class="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center font-bold text-xs"
          >
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="px-4 py-2 hover:bg-slate-800 rounded-xl font-bold text-xs text-slate-400 transition"
          >
            Cancel
          </button>
        </div>
      ...
      <!-- Desktop Table View -->
      <div
        class="hidden lg:block bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden"
      >
        <table class="w-full text-left border-collapse">
          <thead
            class="bg-slate-50 border-b border-slate-100 sticky top-0 z-10 shadow-sm"
          >
            <tr>
              <th class="p-4 w-12">
                <input
              ...
              <th
                v-if="isColumnVisible('orderInfo')"
            ...
            >
              ...
              <td v-if="isColumnVisible('orderInfo')" class="p-4">
                ...
                <div
                  ...
                  class="text-xs text-blue-600 dark:text-blue-400 flex items-center gap-1 mt-1 font-medium"
                >
              ...
              <td v-if="isColumnVisible('status')" class="p-4">
                <div
                  ...
                  "
                  class="relative group/status w-max"
                  @click.stop
                >
                  <select
                    :value="order.status"
              ...
              <td class="p-4 text-right">
                ...
                >
                  ...
                  <button
                    @click="printReceipt(order)"
                    class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-blue-50 dark:hover:bg-slate-700 text-slate-500 hover:text-blue-600 transition"
                    title="Print Receipt"
                  >
                    <span class="material-symbols-outlined text-[20px]"
                      >print</span
      ...
      >
        <p class="text-sm text-slate-500">
          Showing page
          <span class="font-bold text-slate-900">{{ pagination.page }}</span> of
  ...
  >
    <div
      class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
    >
      <!-- Modal Header -->
      <div
        class="p-6 bg-slate-50 border-b border-slate-100 flex items-center justify-between"
      >
        <div>
      ...
      <div class="p-6 overflow-y-auto custom-scrollbar">
        ...
        <div class="flex flex-wrap gap-4 mb-8">
          <div
            class="flex-1 min-w-[140px] bg-slate-50 p-4 rounded-xl border border-slate-100"
          >
            <p
          ...
          >
            ...
            <p class="font-bold text-slate-700 text-sm">
              {{
                (selectedOrder as any).member?.name ||
        ...
        <!-- Totals -->
        <div
          class="flex flex-col items-end gap-2 border-t border-slate-100 pt-6"
        >
          <div class="flex justify-between w-full max-w-xs text-sm">
            <span class="text-slate-500">Subtotal</span>
            <span class="font-bold text-slate-700">{{
              formatCurrency(
                Number((selectedOrder as any).subtotal || selectedOrder.total),
              )
            }}</span>
          </div>
          <div
            v-if="(selectedOrder as any).discount > 0"
            class="flex justify-between w-full max-w-xs text-sm"
          >
            <span class="text-slate-500">Discount</span>
            <span class="font-bold text-blue-600"
              >-{{
                formatCurrency(Number((selectedOrder as any).discount))
              }}</span
            >
          </div>
          <div
            class="flex justify-between w-full max-w-xs text-lg pt-2 mt-2 border-t border-slate-100"
          >
      ...
      <!-- Footer Actions -->
      <div
        class="p-6 bg-slate-50 border-t border-slate-100 grid grid-cols-2 sm:flex sm:justify-end gap-3"
      >
        <button
...
const { needsTenantSelection, showTenantModal, handleTenantSelected } =
  useTenantCheck();
const {
  success: showSuccess,
  error: showError,
    ...
    async () => {
      ...
      try {
        ...
        if (error.response?.status !== 429) {
          ...
          await showError(
            error.response?.data?.message || "Gagal memuat pesanan",
          );
        }
      } finally {
        loading.value = false;
...
const getStatusClass = (status: string) => {
  const classes: Record<string, string> = {
    ...
    PROCESSING:
      "bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800",
...
const printReceipt = async (order: Order) => {
  try {
    ...
    const receiptData = {
      ...
      customerName:
        fullOrder.member?.name ||
        fullOrder.customer?.name ||
        fullOrder.temporaryCustomerName ||
        null,
      memberName: fullOrder.member?.name || null,
      items:
        safeMap(fullOrder.items || [], (item: any) => ({
  ...
  try {
    ...
    if (error.response) {
      if (error.response.data?.message) {
        ...
        errorMessage =
          "Anda tidak memiliki izin untuk mengupdate status pesanan";
      } else if (error.response.status === 400) {
        errorMessage =
          error.response.data?.message || "Status pesanan tidak valid";
      }
    ...
    if (error.response) {
      ...
      if (error.response.data?.message) {
        ...
        errorMessage =
          ...
          "Pesanan tidak dapat dihapus. Pastikan status pesanan adalah CANCELLED atau REFUNDED.";
      }
    } else if (error.message) {
      errorMessage = error.message;
    }
...
const toggleOrderSelection = (order: Order) => {
  ...
  const index = safeFindIndex<Order>(
    selectedOrders.value,
    (o: Order) => o && o.id === order.id,
  );
  if (index > -1) {
...
const canDeleteSelectedOrders = computed(() => {
  ...
  return selectedOrders.value.every(
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (response.data.failed > 0) {
      const errorDetails =
        response.data.errors?.join(", ") || "Beberapa pesanan gagal dihapus";
      await showError(
        `${response.data.failed} pesanan gagal dihapus. ${errorDetails}`,
      );
    }
    ...
    if (error.response) {
      if (error.response.data?.message) {
        ...
        errorMessage =
          error.response.data?.message ||
          "Beberapa pesanan tidak dapat dihapus. Pastikan status pesanan adalah CANCELLED atau REFUNDED.";
      }
    } else if (error.message) {
      errorMessage = error.message;
    }
...
const bulkRefund = async () => {
  ...
  try {
    ...
    if (response.data.failed > 0) {
      const errorDetails =
        response.data.errors?.join(", ") || "Beberapa pesanan gagal direfund";
      await showError(
        `${response.data.failed} pesanan gagal direfund. ${errorDetails}`,
      );
    }
    ...
    if (error.response) {
      if (error.response.data?.message) {
        ...
        errorMessage =
          error.response.data?.message ||
          "Beberapa pesanan tidak dapat direfund. Pastikan status pesanan adalah COMPLETED.";
      }
    } else if (error.message) {
      errorMessage = error.message;
    }
...
const deletableOrdersCount = computed(() => {
  ...
  return safeFilter(
    orders.value,
    (o: any) => o && (o.status === "CANCELLED" || o.status === "REFUNDED"),
  ).length;
});
...
const deleteAllOrders = async () => {
  if (!Array.isArray(orders.value)) return;
  const deletableOrders = orders.value.filter(
    (o) => o.status === "CANCELLED" || o.status === "REFUNDED",
  );

  if (deletableOrders.length === 0) {
    await showError(
      "Tidak ada pesanan yang bisa dihapus. Hanya pesanan dengan status Dibatalkan atau Direfund yang bisa dihapus.",
    );
    return;
  }
  ...
  try {
    ...
    if (response.data.failed > 0) {
      await showError(
        `${response.data.failed} pesanan gagal dihapus. ${response.data.errors.join(", ")}`,
      );
    }
...
const visibleColumns = ref<string[]>(
  JSON.parse(
    localStorage.getItem("ordersVisibleColumns") ||
      '["orderInfo", "customer", "total", "status", "date"]',
  ),
);
