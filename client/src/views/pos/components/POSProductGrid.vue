<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden relative">
    ...
    >
      ...
      <!-- Empty State -->
      <div v-else-if="products.length === 0" class="flex flex-col items-center justify-center h-64 text-slate-500">
      ...
      >
        ...
        >
          ...
          <template v-if="row.data.length < cols">
            <div v-for="i in (cols - row.data.length)" :key="'filler-' + i" class="invisible"></div>
          </template>
        </div>
      </div>
    </div>
...
const rows = computed(() => {
  const result = [];
  for (let i = 0; i < props.products.length; i += cols.value) {
    result.push(props.products.slice(i, i + cols.value));
  }
  return result;
});
