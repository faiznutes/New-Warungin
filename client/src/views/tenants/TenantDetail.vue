<template>
  <div
    class="bg-[#f6f7f8] dark:bg-[#101922] min-h-screen font-display flex flex-col h-screen overflow-hidden animate-fade-in"
  >
    <!-- Header -->
    <header
      class="h-16 flex items-center justify-between px-6 lg:px-8 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 shrink-0 z-10 sticky top-0"
    >
      <div class="flex items-center gap-2 text-sm">
        <a
          href="#"
          @click.prevent="handleBackToTenants"
          class="text-slate-500 hover:text-blue-600 font-bold transition-colors"
          >Tenants</a
        >
        <span class="text-slate-300 material-symbols-outlined text-[16px]"
          >chevron_right</span
        >
    ...
    <div class="flex-1 overflow-y-auto p-6 lg:p-8 custom-scrollbar">
      <div class="w-full flex flex-col gap-6">
        ...
        <div
          v-if="loading"
          class="flex flex-col items-center justify-center min-h-[400px] bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700"
        >
          <div
            class="animate-spin rounded-full h-12 w-12 border-4 border-blue-600 border-t-transparent"
          ></div>
          <p
            class="mt-4 text-slate-500 dark:text-slate-400 font-bold animate-pulse"
          >
        ...
        >
          <span class="material-symbols-outlined text-4xl text-red-500 mb-2"
            >error</span
          >
          ...
          <button
            ...
            class="px-5 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 transition font-bold flex items-center gap-2 mx-auto shadow-sm"
          >
        ...
        <template v-else>
          <!-- Hero Card -->
          <section
            class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden group hover:shadow-md transition-all"
          >
            <div
              class="h-32 bg-gradient-to-r from-blue-600 to-indigo-600 relative overflow-hidden"
            >
            ...
            <div class="px-6 pb-6 lg:px-8">
              <div
                class="relative flex flex-col lg:flex-row gap-6 items-start -mt-12"
              >
                <!-- Tenant Logo -->
                <div class="shrink-0 relative">
                  <div
                    class="w-28 h-28 rounded-2xl bg-white dark:bg-slate-800 shadow-lg border-4 border-white dark:border-slate-700 p-1"
                  >
                    <div
                      class="w-full h-full rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-900 flex items-center justify-center text-5xl font-black text-slate-300 dark:text-slate-600 select-none"
                    >
                      {{ tenant?.name?.charAt(0).toUpperCase() }}
                    </div>
                  </div>
                  <div
                    class="absolute bottom-1 right-1 translate-x-1/4 translate-y-1/4"
                  >
                    <span
                      class="flex h-6 w-6 items-center justify-center rounded-full ring-4 ring-white dark:ring-slate-800 shadow-sm"
                      :class="tenant?.isActive ? 'bg-blue-500' : 'bg-red-500'"
                    >
                      <span
                        class="material-symbols-outlined text-white text-[14px] font-bold"
                        >{{ tenant?.isActive ? "check" : "close" }}</span
                      >
                    </span>
                  </div>
                </div>
                ...
                <div class="flex-1 pt-2 lg:pt-14 min-w-0">
                  ...
                  >
                    <div>
                      <h1
                        class="text-3xl font-black text-slate-900 dark:text-white leading-tight flex items-center gap-2 tracking-tight"
                      >
                        {{ tenant?.name }}
                        <span
                          v-if="tenant?.isActive"
                          class="material-symbols-outlined text-blue-500 text-[24px] icon-filled"
                          title="Verified Tenant"
                          >verified</span
                        >
                      </h1>
                      <div
                        class="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2 text-sm text-slate-500 dark:text-slate-400 font-medium"
                      >
                        <span
                          class="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700/50 px-2 py-0.5 rounded-lg"
                        >
                        ...
                        >
                          <span
                            ...
                            :class="
                              tenant?.isActive ? 'bg-blue-500' : 'bg-red-500'
                            "
                          ></span>
                    ...
                    <!-- Quick Stats -->
                    <div
                      class="flex items-center gap-8 bg-slate-50 dark:bg-slate-700/30 px-6 py-3 rounded-2xl border border-slate-100 dark:border-slate-700/50"
                    >
                      <div class="flex flex-col items-center">
                        ...
                        <span
                          ...
                          >{{ tenantStores.length }}</span
                        >
                      </div>
                      <div
                        class="w-px h-8 bg-slate-200 dark:bg-slate-600"
                      ></div>
                      <div class="flex flex-col items-center">
                        <span
                          class="text-[10px] font-bold uppercase tracking-wider text-slate-400"
                        ...
                        >
                        <span
                          class="text-sm font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2.5 py-0.5 rounded-lg border border-blue-100 dark:border-blue-800"
                        >
                          {{
                            getPlanName(
                              subscription?.plan ||
                ...
                <!-- Actions -->
                <div
                  class="w-full lg:w-auto flex items-center gap-3 pt-2 lg:pt-14"
                >
                  <button
                    @click="loadTenantDetail"
                  ...
                  >
                    <span
                      class="material-symbols-outlined text-[20px] group-hover/btn:rotate-180 transition-transform duration-500"
                      >refresh</span
                    >
                    Refresh
                  </button>
            ...
            <!-- Tabs -->
            <div
              class="mt-2 px-6 lg:px-8 border-t border-slate-100 dark:border-slate-700"
            >
              <div
                class="flex overflow-x-auto gap-8 no-scrollbar scroll-smooth"
              >
                <a
                  v-for="tab in [
                    'profile',
                ...
                >
                  <span
                    class="text-sm tracking-wide transition-all capitalize"
                    :class="
                      activeTab === tab
                        ? 'font-black text-blue-600 scale-105'
                        : 'font-bold text-slate-500 group-hover:text-slate-900 dark:text-slate-400 dark:group-hover:text-white'
                    "
                  >
                    {{ getTabLabel(tab) }}
                  </span>
                  <span
                    class="absolute bottom-0 h-[3px] w-full rounded-t-full transition-all duration-300"
                    :class="
                      activeTab === tab
                        ? 'bg-blue-600 w-full'
                        : 'bg-transparent w-0 group-hover:bg-slate-200 dark:group-hover:bg-slate-700 group-hover:w-full'
                    "
                  ></span>
                </a>
              </div>
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <span
                class="text-sm font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full"
                >Terakhir diperbarui:
                {{
                  tenant?.updatedAt ? formatDate(tenant.updatedAt) : "Baru saja"
                }}</span
              >
            </div>
            <div
              class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden"
            >
              <div
                class="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10"
              >
                <!-- Item: Owner Name -->
                <div class="flex flex-col gap-2 relative group">
                  ...
                  <p
                    class="text-lg font-bold text-slate-900 dark:text-white border-b-2 border-slate-100 dark:border-slate-700 pb-2 group-hover:border-blue-500 transition-colors"
                  >
                    {{ tenant?.name }}
                  </p>
                </div>
                ...
                <div class="flex flex-col gap-2 relative group">
                  ...
                  <p
                    class="text-lg font-bold text-slate-900 dark:text-white border-b-2 border-slate-100 dark:border-slate-700 pb-2 group-hover:border-blue-500 transition-colors break-all"
                  >
                    {{ tenant?.email }}
                  </p>
                </div>
              ...
              <div
                class="bg-slate-50/50 dark:bg-slate-800/50 px-8 py-5 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between flex-wrap gap-4"
              >
                <span class="text-sm font-medium text-slate-500"
                  >Perlu mengupdate informasi tenant?</span
                >
                <div class="flex gap-3">
                  <button
                    @click="openEditProfileModal"
                    class="px-4 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors flex items-center gap-2 shadow-sm"
                  >
                  ...
                  <button
                    @click="handleRequestUpdateProfile"
                    class="text-sm font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors hover:underline"
                  >
                    Request ke Tenant
                  </button>
                </div>
          ...
          <section
            ...
            class="flex flex-col gap-8 animate-fade-in-up"
          >
            <div class="flex items-center justify-between">
              <h2 class="text-xl font-black text-slate-900 dark:text-white">
                Manajemen Langganan
              ...
              <div
                class="flex items-center gap-2 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm"
              >
              ...
              <div class="p-8 flex flex-col lg:flex-row gap-10">
                ...
                <div class="flex-1 lg:max-w-md">
                  <div class="flex items-start gap-5 mb-8">
                    <div
                      class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white shrink-0 shadow-lg shadow-blue-500/30"
                    >
                    ...
                    <div>
                      <div class="flex items-center gap-3 mb-1">
                        ...
                        <span
                          class="inline-flex items-center gap-1 rounded-lg bg-blue-50 dark:bg-blue-900/20 px-2.5 py-1 text-xs font-bold text-blue-700 dark:text-blue-400 ring-1 ring-blue-100 dark:ring-blue-800"
                        >
                          Aktif
                        </span>
                      </div>
                      <p
                        class="text-slate-500 text-sm font-medium leading-relaxed"
                      >
                        {{
                          subscription?.plan === "ENTERPRISE"
                            ? "Mencakup akses tanpa batas dan API kustom."
                            : subscription?.plan === "PRO"
                              ? "Mencakup analitik canggih dan dukungan prioritas."
                              : "Fitur inti untuk bisnis kecil."
                        }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    class="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-700 flex items-center justify-between"
                  >
                    <div>
                      <span
                        class="text-[10px] uppercase font-bold text-slate-400 block mb-1"
                        >Pembayaran Rutin</span
                      >
                    ...
                    <div
                      class="h-10 w-px bg-slate-200 dark:border-slate-700 mx-2"
                    ></div>
                ...
                <div
                  class="w-px bg-slate-200 dark:border-slate-700 hidden lg:block"
                ></div>
                ...
                <div class="flex-1 flex flex-col justify-between gap-8">
                  <div class="grid grid-cols-2 gap-y-6 gap-x-8 text-sm">
                    ...
                    <div>
                      ...
                      <div
                        class="flex items-center gap-2 font-bold text-slate-900 dark:text-white"
                      >
                        <div
                          class="h-6 bg-white border border-slate-200 rounded px-1.5 flex items-center justify-center shadow-sm"
                        >
                          <span
                            class="text-[10px] font-black text-blue-800 italic"
                            >VISA</span
                          >
                        </div>
                        •••• 4242
                    ...
                    <div>
                      ...
                      <div class="flex items-center gap-3">
                        <span
                          ...
                          >{{ daysRemainingDisplay }}</span
                        >
                        <div
                          class="w-24 h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden"
                        >
                          <div
                  ...
                  <div class="flex items-end justify-end mt-auto gap-3">
                    <button
                      @click="handleEditSubscription"
                      class="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold shadow-lg shadow-blue-500/30 transition-all flex items-center justify-center gap-2 hover:-translate-y-0.5"
                    >
            ...
            <div class="flex flex-col gap-4">
              <div class="flex items-center justify-between">
                ...
                <button
                  @click="handleViewAllInvoices"
                  class="text-sm font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1 transition-colors hover:gap-2"
                >
                  Lihat semua invoice
                  <span class="material-symbols-outlined text-[18px]"
                    >arrow_forward</span
              ...
              >
                <div class="overflow-x-auto">
                  <table class="w-full text-sm text-left">
                    <thead
                      class="text-[10px] text-slate-500 uppercase bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 font-bold tracking-wider"
                    >
                      <tr>
                    ...
                    >
                      <template v-if="loadingBilling">
                        <tr v-for="i in 3" :key="i" class="animate-pulse">
                          <td colspan="7" class="px-6 py-4">
                            <div
                              class="h-4 bg-slate-100 dark:bg-slate-700 rounded w-full"
                            ></div>
                          </td>
                        </tr>
                      </template>
                      ...
                      >
                        <td
                          class="px-6 py-4 font-bold text-slate-900 dark:text-white font-mono"
                        >
                          #{{
                        ...
                        <td
                          class="px-6 py-4 text-slate-900 dark:text-white font-bold"
                        >
                          {{ getPlanName(invoice.planType || invoice.plan) }}
                        </td>
                        <td
                          class="px-6 py-4 text-slate-900 dark:text-white font-mono"
                        >
                          Rp
                          {{
                        ...
                        <td class="px-6 py-4">
                          <span
                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase transition-colors shadow-sm"
                            :class="
                              invoice.status === 'ACTIVE' ||
                              invoice.reverted === false
                                ? 'bg-blue-50 text-blue-700 border border-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800'
                        ...
                        <td class="px-6 py-4 text-right">
                          <button
                            ...
                            class="text-slate-400 hover:text-blue-600 transition-colors inline-flex items-center justify-center p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800"
                            title="Download PDF"
                          >
                            <span class="material-symbols-outlined text-[20px]"
                              >download</span
          ...
          >
            ...
            >
              <div>
                ...
                <p class="text-sm text-slate-500 font-medium mt-1">
                  Kelola fitur premium yang diaktifkan untuk tenant ini.
                </p>
              </div>
              <button
                @click="showAddAddonModal = true"
                class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-lg shadow-blue-500/30 hover:-translate-y-0.5"
              >
                <span class="material-symbols-outlined text-[20px]"
                  >add_circle</span
                >
            ...
            <div
              v-if="filteredActiveAddons.length === 0"
              class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 p-16 text-center"
            >
              <div
                class="w-20 h-20 bg-slate-50 dark:bg-slate-700/50 rounded-full flex items-center justify-center mx-auto mb-6"
              >
                <span
                  class="material-symbols-outlined text-slate-300 text-[40px]"
                  >extension_off</span
                >
              </div>
              <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">
                Belum ada penyaya aktif
              </h3>
              <p class="text-slate-500 text-sm max-w-xs mx-auto font-medium">
                Tenant ini belum mengaktifkan fitur premium apapun.
              </p>
            </div>
            ...
            >
              ...
              >
                ...
                <div class="flex justify-between items-start mb-4">
                  <div class="flex items-center gap-4">
                    <div
                      ...
                      :class="getAddonColor(addon, 'bg')"
                    >
                      <span
                        class="material-symbols-outlined text-[28px]"
                        :class="getAddonColor(addon, 'text')"
                        >{{ getAddonIcon(addon) }}</span
                      >
                    </div>
                    <div>
                      <h3
                      ...
                      <div class="flex items-center gap-2 mt-1">
                        ...
                        >
                        <span
                          class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase"
                          :class="
                            addon.purchasedBy === 'ADMIN'
                ...
                <p
                  class="text-sm text-slate-500 mb-6 leading-relaxed line-clamp-3 flex-1 font-medium"
                >
                  {{
                    getAddonDescription(addon) ||
                    "Fitur lanjutan untuk operasional bisnis Anda."
                  }}
                </p>
                <!-- Limit Bar -->
                <div v-if="addon.limit && addon.limit !== -1" class="mb-6">
                  <div
                    class="flex items-center justify-between text-[10px] uppercase font-bold tracking-wider mb-2"
                  >
                    <span class="text-slate-400">Penggunaan</span>
                    <span
                      :class="
                        addon.isLimitReached
                          ? 'text-red-600'
                  ...
                  >
                    <div
                      ...
                      :class="
                        addon.isLimitReached ? 'bg-red-500' : 'bg-blue-500'
                      "
                      :style="{
                        width: `${Math.min(100, ((addon.currentUsage || 0) / (addon.limit || 1)) * 100)}%`,
                      }"
                    ></div>
                  </div>
                </div>
                <div
                  class="flex gap-3 mt-auto pt-4 border-t border-slate-100 dark:border-slate-700"
                >
                  <button
                    @click="handleEditAddon(addon)"
                    class="flex-1 py-2.5 px-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-bold text-slate-600 dark:text-slate-300 transition-colors flex items-center justify-center gap-2"
                  >
                    <span class="material-symbols-outlined text-[18px]"
                      >edit</span
                    >
                  ...
                  <button
                    @click="handleDeactivateAddon(addon)"
                    class="py-2.5 px-3 rounded-xl border border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-900/20 text-xs font-bold text-red-600 dark:text-red-400 transition-colors flex items-center justify-center gap-2"
                  >
                    <span class="material-symbols-outlined text-[18px]"
                      >block</span
                    >
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <button
                ...
                class="px-4 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors flex items-center gap-2 shadow-lg shadow-blue-500/30"
              >
            ...
            >
              <div v-if="tenantUsers.length === 0" class="p-12 text-center">
                <span
                  class="material-symbols-outlined text-5xl text-slate-300 mb-4"
                  >group</span
                >
                ...
                <button
                  @click="showAddUserModal = true"
                  class="px-4 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors"
                >
                  Tambah User Pertama
                </button>
              </div>
                ...
                <table class="w-full text-sm text-left">
                  ...
                  >
                    ...
                    >
                      ...
                      <td class="px-6 py-4">
                        <span
                          class="px-2 py-1 bg-blue-50 text-blue-700 text-xs font-bold rounded-lg"
                          >{{ user.role }}</span
                        >
                      </td>
                      <td class="px-6 py-4">
                      ...
                      <td class="px-6 py-4 text-right">
                        ...
                        >
                          ...
                          <button
                            ...
                            class="p-1.5 hover:bg-amber-50 text-slate-400 hover:text-amber-600 rounded-lg transition-colors"
          ...
          >
            ...
            <div
              v-else-if="tenantStores.length === 0"
              class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 p-12 text-center"
            >
              <span
                class="material-symbols-outlined text-5xl text-slate-300 mb-4"
                >store</span
              ...
              <p class="text-slate-500 font-medium mb-4">
                Tenant ini belum memiliki toko terdaftar.
              </p>
              <button
                @click="showAddStoreModal = true"
                class="px-4 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors"
            ...
            >
              ...
              >
                <div class="flex items-start justify-between mb-4">
                  <div
                    class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white shadow-lg shadow-blue-500/30"
                  >
                    <span class="material-symbols-outlined">storefront</span>
                  </div>
                  <span
                    class="flex items-center gap-1.5 text-xs font-bold px-2 py-1 rounded-lg"
                    :class="
                      store.isActive
                  ...
                  >
                    <span
                      class="w-1.5 h-1.5 rounded-full"
                      :class="store.isActive ? 'bg-green-500' : 'bg-red-500'"
                    ></span>
                    {{ store.isActive ? "Aktif" : "Nonaktif" }}
                  </span>
                </div>
                <h3
                  class="font-bold text-lg text-slate-900 dark:text-white mb-1"
                >
                  {{ store.name }}
                </h3>
                <p
                  class="text-sm text-slate-500 font-medium flex items-center gap-1.5"
                >
                  <span class="material-symbols-outlined text-[16px]"
                    >location_on</span
                  >
                ...
                <div
                  v-if="store.phone"
                  class="text-sm text-slate-500 font-medium flex items-center gap-1.5 mt-1"
                >
                  <span class="material-symbols-outlined text-[16px]"
                    >call</span
                  >
                ...
                <!-- Action Buttons -->
                <div
                  class="flex gap-2 mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 opacity-0 group-hover:opacity-100 transition-opacity"
                >
                  <button
                    @click="handleEditStore(store)"
                    class="flex-1 py-2 px-3 text-xs font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors flex items-center justify-center gap-1"
                  >
                    <span class="material-symbols-outlined text-[16px]"
                      >edit</span
                    >
                  ...
                  <button
                    @click="handleToggleStoreStatus(store)"
                    class="flex-1 py-2 px-3 text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-1"
                    :class="
                      store.isActive
                  ...
                  >
                    <span class="material-symbols-outlined text-[16px]">{{
                      store.isActive ? "block" : "check_circle"
                    }}</span>
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto"
        >
          ...
          <p class="text-sm text-slate-600 dark:text-slate-400 mb-4">
            Tambah user baru untuk tenant ini. User akan dibuat dengan password
            default yang dapat diubah kemudian.
          </p>
          <form @submit.prevent="handleAddUserSubmit" class="space-y-4">
            ...
            >
              <p class="text-xs text-blue-700 dark:text-blue-300">
                <span class="font-bold">ℹ️ Info:</span> Password default akan
                di-generate otomatis. User dapat mengubahnya setelah login
                pertama.
              </p>
            </div>
            <div class="flex gap-3 pt-4">
              <button
                type="button"
                @click="showAddUserModal = false"
                class="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700"
              >
                Batal
              </button>
              <button
                type="submit"
                :disabled="saving"
                class="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                <span
                  v-if="!saving"
                  class="material-symbols-outlined text-[18px]"
    ...
    <Teleport to="body">
      <div
        ...
        class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
        @click.self="showAddStoreModal = false"
      >
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto"
        >
          ...
          <p class="text-slate-500 dark:text-slate-400 mb-6 text-sm">
            Lengkapi informasi toko baru untuk tenant ini. Semua field yang
            wajib diisi sudah ditandai dengan (*).
          </p>
          <form @submit.prevent="handleAddStore" class="space-y-5">
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    type="text"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Contoh: Warungin Pusat"
                  />
                </div>
                <div>
                  <label
                  ...
                  <textarea
                    ...
                    rows="3"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            ...
            <div class="border-b border-slate-200 dark:border-slate-700 pb-5">
              ...
              <div class="space-y-3">
                ...
                >
                  <label
                    class="w-20 text-sm font-medium text-slate-600 dark:text-slate-300"
                    >{{ day }}</label
                  >
                  <div class="flex items-center gap-2">
                    <input
                      type="checkbox"
                      v-model="
                        newStoreForm.operatingHours[day.toLowerCase()].isOpen
                      "
                      class="rounded"
                    />
                    <input
                      v-model="
                        newStoreForm.operatingHours[day.toLowerCase()].open
                      "
                      type="time"
                      :disabled="
                        !newStoreForm.operatingHours[day.toLowerCase()].isOpen
                      "
                      class="px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm disabled:opacity-50"
                    />
                    <span class="text-slate-500">-</span>
                    <input
                      v-model="
                        newStoreForm.operatingHours[day.toLowerCase()].close
                      "
                      type="time"
                      :disabled="
                        !newStoreForm.operatingHours[day.toLowerCase()].isOpen
            ...
            <div class="pb-5">
              <div class="flex items-center justify-between mb-4">
                ...
                >
                  👥 Konfigurasi Shift (Opsional)
                </h4>
                <button
                  type="button"
                  @click="addShift"
                  class="text-xs px-3 py-1.5 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors"
                >
                  + Tambah Shift
                </button>
              </div>
              <div
                v-if="newStoreForm.shiftConfig.length === 0"
                class="text-sm text-slate-500 dark:text-slate-400 text-center py-4 border border-dashed border-slate-300 dark:border-slate-600 rounded-lg"
              >
                Belum ada shift. Klik "+ Tambah Shift" untuk menambahkan.
              </div>
              <div v-else class="space-y-3">
                <div
                  v-for="(shift, index) in newStoreForm.shiftConfig"
                  :key="index"
                  class="p-3 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 space-y-3"
                >
                  <div class="flex items-center gap-3">
                    <input
                      v-model="shift.name"
                      type="text"
                      placeholder="Nama shift (Pagi, Siang, Malam, etc)"
                      class="flex-1 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm"
                    />
                    <button
                      type="button"
                  ...
                  <div class="flex items-center gap-3">
                    <span
                      class="text-xs text-slate-500 dark:text-slate-400 w-20"
                      >Jam Mulai</span
                    >
                    <input
                    ...
                    />
                    <span
                      class="text-xs text-slate-500 dark:text-slate-400 w-20 text-right"
                      >Jam Selesai</span
                    >
                    <input
            ...
            >
              ...
              <button
                type="submit"
                :disabled="saving || !newStoreForm.name"
                class="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
              >
                <span
                  v-if="saving"
    ...
    <Teleport to="body">
      <div
        ...
        class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
        @click.self="showEditStoreModal = false"
      >
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto"
        >
          ...
          <form @submit.prevent="handleSaveStore" class="space-y-5">
            ...
            <div class="border-b border-slate-200 dark:border-slate-700 pb-5">
              ...
              <div class="space-y-4">
                ...
                <div>
                  ...
                  >
                  <select
                    v-model="editStoreForm.isActive"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm"
                  >
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    <input
                      type="checkbox"
                      v-model="
                        editStoreForm.operatingHours[day.toLowerCase()].isOpen
                      "
                      class="rounded"
                    />
                    <input
                      v-model="
                        editStoreForm.operatingHours[day.toLowerCase()].open
                      "
                      type="time"
                      :disabled="
                        !editStoreForm.operatingHours[day.toLowerCase()].isOpen
                    ...
                    <input
                      v-model="
                        editStoreForm.operatingHours[day.toLowerCase()].close
                      "
                      type="time"
                      :disabled="
                        !editStoreForm.operatingHours[day.toLowerCase()].isOpen
            ...
            <div class="pb-5">
              ...
              <div
                v-if="editStoreForm.shiftConfig.length === 0"
                class="text-sm text-slate-500 dark:text-slate-400 text-center py-4 border border-dashed border-slate-300 dark:border-slate-600 rounded-lg"
              >
                Belum ada shift. Klik "+ Tambah Shift" untuk menambahkan.
              </div>
              <div v-else class="space-y-3">
                <div
                  v-for="(shift, index) in editStoreForm.shiftConfig"
                  :key="index"
                  class="p-3 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 space-y-3"
                >
                  <div class="flex items-center gap-3">
    ...
    >
      <div
        class="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl"
      >
        <div class="flex items-center gap-3 mb-4">
          <span class="material-symbols-outlined text-blue-500 text-3xl"
            >extension</span
        ...
        <form @submit.prevent="handleAddAddonSubmit" class="space-y-4">
          <div>
            ...
            >
            <select
              v-model="newAddonForm.selectedAddonId"
              required
              class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm"
            >
              <option value="" disabled>Pilih addon...</option>
              <option
                v-for="opt in availableAddonsList"
                :key="opt.id"
                :value="opt.id"
              >
            ...
            <p v-if="selectedNewAddon" class="text-xs text-slate-500 mt-1">
              {{ selectedNewAddon.description }}
            </p>
          </div>
          <div>
            <label
            ...
            <p class="text-xs text-slate-500 mb-1">
              Durasi aktif addon dihitung mulai hari ini.
            </p>
            <input
              v-model.number="newAddonForm.durationDays"
              type="number"
              min="1"
              placeholder="30"
              required
          ...
          <div class="flex justify-end gap-3 pt-4">
            <button
              ...
              class="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
            >
              Batal
            </button>
            <button
              ...
              :disabled="
                saving ||
                !newAddonForm.selectedAddonId ||
                selectedNewAddon?.comingSoon
              "
              class="px-4 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2"
            >
              <span
                v-if="saving"
                class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto"
      >
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-slate-900 dark:text-white">
            Edit Profil Tenant
        ...
        <form @submit.prevent="handleSaveProfile" class="space-y-4">
          ...
          <div class="flex gap-3 pt-4">
            <button
              type="button"
              @click="showEditProfileModal = false"
              class="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50"
            >
              Batal
            </button>
            <button
      ...
      >
        ...
        <form @submit.prevent="handleSaveAddon" class="space-y-4">
          ...
          <div>
            ...
            >
            <p class="text-xs text-slate-500 mb-1">
              Masukkan jumlah hari untuk memperpanjang addon dari HARI INI.
            </p>
            <input
              v-model.number="editAddonForm.durationDays"
              type="number"
              min="1"
              placeholder="30"
              class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm"
...
const progressWidth = computed(() => {
  const startRaw =
    subscription.value?.subscriptionStart || tenant.value?.subscriptionStart;
  const endRaw =
    subscription.value?.subscriptionEnd || tenant.value?.subscriptionEnd;
...
const updateCountdown = () => {
  const endDateStr =
    subscription.value?.subscriptionEnd ||
    subscription.value?.endDate ||
    tenant.value?.subscriptionEnd;
  if (!endDateStr) {
...
const selectedNewAddon = computed(() => {
  ...
  return (
    availableAddonsList.value.find(
      (a: any) => a.id === newAddonForm.value.selectedAddonId,
    ) || null
  );
});
...
const handleSaveAddon = async () => {
  if (
    !editAddonForm.value.durationDays ||
    editAddonForm.value.durationDays < 1
  ) {
    showError("Durasi addon minimal 1 hari.");
    return;
  }
...
const handleToggleUserStatus = async (user: any) => {
  if (user.role === "SUPER_ADMIN") {
    showError(
      "User SUPER_ADMIN tidak dapat dinonaktifkan dari halaman tenant.",
    );
    return;
  }
...
const handleDeleteUser = async (user: any) => {
  if (user.role === "SUPER_ADMIN") {
    showError("User SUPER_ADMIN tidak dapat dihapus dari halaman tenant.");
    return;
  }

  const confirmed = await confirmDialog(
    `Apakah Anda yakin ingin menghapus user "${user.name}"? Aksi ini tidak dapat dibatalkan.`,
    "Hapus User",
    "Hapus",
    "Batal",
  );
...
const handleToggleStoreStatus = async (store: any) => {
  ...
  const confirmed = await confirmDialog(
    `Apakah Anda yakin ingin ${action} toko "${store.name}"?`,
    "Konfirmasi",
    "Ya",
    "Batal",
  );
...
const loadTenantDetail = async () => {
  ...
  try {
    ...
    hasError.value = true;
    errorMessage.value =
      error.response?.data?.message || "Gagal memuat detail tenant";
...
const getAddonColor = (addon: any, type: "bg" | "text") => {
  const colors: Record<string, any> = {
    ...
    payment_accounting_integration: {
...
const getAddonIcon = (addon: any) => {
  const icons: Record<string, string> = {
    whatsapp_integration: "message",
    advanced_analytics: "query_stats",
    hardware_sync: "print",
    loyalty_engine: "redeem",
    offline_sync_pro: "cloud_off",
    custom_mobile_app: "smartphone",
...
const getAddonDescription = (addon: any) => {
  ...
  const match = availableAddonsList.value.find(
    (a: any) => a.id === addon.addonId || a.id === addon.id,
  );
...
const filteredActiveAddons = computed(() =>
  activeAddons.value.filter((a) => a.status?.toUpperCase() === "ACTIVE"),
);
...
const handleAddUserSubmit = async () => {
  ...
  try {
    ...
    const defaultPassword =
      response.data?.data?.defaultPassword || response.data?.defaultPassword;
    if (defaultPassword) {
      showSuccess(
        `User berhasil ditambahkan. Password default: ${defaultPassword}`,
      );
    } else {
...
const handleSaveUserFromModal = async (userData: any) => {
  if (
    editingUser.value?.role === "SUPER_ADMIN" &&
...
const handleEditStore = (store: any) => {
  ...
  editStoreForm.value = {
    ...
    operatingHours:
      store.operatingHours && typeof store.operatingHours === "object"
...
const addEditShift = () => {
  editStoreForm.value.shiftConfig.push({
    name: "",
    startTime: "08:00",
    endTime: "17:00",
  });
