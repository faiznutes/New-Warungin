<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      <div>
        <h1
          class="text-[#0d141b] dark:text-white text-3xl font-bold leading-tight tracking-tight"
        >
          Support Tickets
        </h1>
        <p class="text-[#4c739a] dark:text-slate-400 mt-2">
          View and manage tenant support requests, track issues, and assign
          agents.
        </p>
      </div>
      <div class="flex gap-3">
        <div
          class="bg-white dark:bg-slate-800 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-3"
        >
          ...
          <div class="flex flex-col">
            <span
              class="text-xs text-slate-500 dark:text-slate-400 font-medium uppercase"
              >High Priority</span
            >
            <span class="text-sm font-bold text-[#0d141b] dark:text-white"
              >{{ highPriorityCount }} Active</span
            >
          </div>
        </div>
        <div
    ...
    <!-- Filter Bar -->
    <div
      class="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col xl:flex-row gap-4 justify-between items-center z-20"
    >
      <div class="flex flex-col md:flex-row gap-3 w-full xl:w-auto flex-1">
        <div class="relative w-full md:w-80">
          <span
            class="material-symbols-outlined absolute left-3 top-2.5 text-slate-400 text-[20px]"
            >search</span
          >
          <input
            v-model="searchQuery"
        ...
        <div class="flex gap-3 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          <select
            ...
            @change="loadTickets"
            class="form-select text-sm rounded-xl border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:ring-primary min-w-[140px] py-2.5 cursor-pointer hover:border-slate-300 dark:hover:border-slate-500 transition-colors"
          >
      ...
      <button
        ...
        class="w-full xl:w-auto px-5 py-2.5 bg-primary hover:bg-blue-600 text-white rounded-xl text-sm font-semibold shadow-lg shadow-blue-500/30 transition-all flex items-center justify-center gap-2 whitespace-nowrap"
      >
    ...
    <div
      v-else
      class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden"
    >
            ...
            >
              ...
              <td class="px-6 py-4">
                ...
                >
                  <span
                    ...
                    :class="getStatusDotClass(ticket.status)"
                  ></span>
              ...
              <td class="px-6 py-4">
                <div v-if="ticket.assignedTo" class="flex items-center gap-2">
                  <div
                    class="size-7 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-bold"
                  >
              ...
              <td class="px-6 py-4">
                ...
                >
                  <button
                    ...
                    class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-500 hover:text-primary transition-colors"
                    title="View Details"
                  >
                  ...
                  <button
                    ...
                    class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-500 hover:text-blue-600 transition-colors"
                    title="Assign Agent"
                  >
                    <span class="material-symbols-outlined text-[18px]"
                      >person_add</span
                  ...
                  <button
                    @click="addNote(ticket)"
                    class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-500 hover:text-green-600 transition-colors"
                    title="Add Note"
                  >
                    <span class="material-symbols-outlined text-[18px]"
                      >add_comment</span
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-lg w-full"
        >
          <div
          ...
          <form @submit.prevent="submitTicket" class="p-6 space-y-4">
            <div>
              ...
              <input
                ...
                required
                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900/50 text-sm"
            ...
            <div>
              ...
              <textarea
                ...
                required
                rows="4"
                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900/50 text-sm resize-none"
            ...
            <div class="flex gap-3 pt-4">
              ...
              <button
                type="submit"
                :disabled="submitting"
                class="flex-1 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-medium hover:bg-blue-600 disabled:opacity-50"
              >
    ...
    <Teleport to="body">
      <div
        v-if="showNoteModal && selectedTicket"
        class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div class="p-6 space-y-4">
            ...
            <div class="flex gap-3">
              ...
              <button
                ...
                :disabled="!noteContent || addingNote"
                class="flex-1 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-medium hover:bg-blue-600 disabled:opacity-50"
              >
...
const confirmAssign = async () => {
  ...
  try {
    await api.put(`/support/tickets/${selectedTicket.value.id}/assign`, {
      agentId: assigneeId.value,
    });
...
const confirmAddNote = async () => {
  ...
  try {
    await api.post(`/support/tickets/${selectedTicket.value.id}/notes`, {
