  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-gray-600">
          Prediksi penjualan, rekomendasi produk, segmentasi pelanggan, dan optimasi harga
        </p>
      </div>
    </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-green-50 rounded-lg p-4 border-l-4 border-green-500">
              <p class="text-sm text-gray-600 mb-1">
                Total Pendapatan yang Diprediksi
              </p>
              <p class="text-2xl font-bold text-green-600">
                Rp {{ formatCurrency(salesForecastSummary.totalRevenue) }}
              </p>
            </div>
            <div class="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
              ...
              <p class="text-2xl font-bold text-blue-600">
                Rp {{ formatCurrency(salesForecastSummary.averageMonthly) }}
              </p>
            </div>
            <div class="bg-purple-50 rounded-lg p-4 border-l-4 border-purple-500">
              ...
              <p class="text-2xl font-bold text-purple-600">
                {{ (salesForecastSummary.growthRate * 100).toFixed(1) }}%
              </p>
            </div>
            <div class="bg-yellow-50 rounded-lg p-4 border-l-4 border-yellow-500">
              ...
              <p class="text-2xl font-bold text-yellow-600">
                {{ (salesForecastSummary.confidence * 100).toFixed(0) }}%
              </p>
            </div>
          </div>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr
                  v-for="(forecast, index) in salesForecast"
                  :key="index"
                >
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    Rp {{ formatCurrency(forecast.forecastedRevenue) }}
                  </td>
                  <td
                    class="px-6 py-4 whitespace-nowrap text-sm"
                    :class="forecast.growthRate >= 0 ? 'text-green-600' : 'text-red-600'"
                  >
    ...
    >
      <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div
          v-if="productRecommendations.length > 0"
    ...
    >
      <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div
          v-if="customerSegments.length > 0"
          class="space-y-4"
        >
          <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <p class="text-lg font-semibold text-gray-900">
                  Rp {{ formatCurrency(segment.totalRevenue) }}
                </p>
              </div>
              <div>
                <p class="text-sm text-gray-600">
                ...
                <p class="text-lg font-semibold text-gray-900">
                  Rp {{ formatCurrency(segment.averageOrderValue) }}
                </p>
              </div>
              <div>
                <p class="text-sm text-gray-600">
                ...
                <p class="text-lg font-semibold text-gray-900">
                  {{ segment.purchaseFrequency.toFixed(1) }}x/month
                </p>
              </div>
            </div>

            <div
              v-if="segment.topCustomers && segment.topCustomers.length > 0"
              class="mt-4"
            >
    ...
    >
      <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div
          v-if="priceOptimizations.length > 0"
          class="space-y-4"
        >
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr
                  v-for="opt in priceOptimizations"
                  :key="opt.productId"
                >
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    Rp {{ formatCurrency(opt.currentPrice) }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-green-600">
                    Rp {{ formatCurrency(opt.recommendedPrice) }}
                  </td>
                  <td
                    class="px-6 py-4 whitespace-nowrap text-sm"
                    :class="opt.priceChange >= 0 ? 'text-green-600' : 'text-red-600'"
                  >
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm">
                      <p class="text-gray-900">
                        Pendapatan: <span
                          class="font-semibold"
                          :class="opt.expectedRevenueChange >= 0 ? 'text-green-600' : 'text-red-600'"
                        >
                      ...
                      <p class="text-gray-600">
                        Laba: <span
                          class="font-semibold"
                          :class="opt.expectedProfitChange >= 0 ? 'text-green-600' : 'text-red-600'"
                        >
