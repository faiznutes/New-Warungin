<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-900 font-display">
    ...
    <div class="fixed inset-0 pointer-events-none overflow-hidden">
      <div
        class="absolute top-0 right-1/4 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute bottom-0 left-1/4 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl"
      ></div>
    </div>
    ...
    <div class="relative z-10 p-6 lg:p-8 flex flex-col gap-8 w-full">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div class="flex flex-col gap-2 animate-fade-in-down">
          <h1
            class="text-3xl sm:text-4xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-700 dark:from-white dark:to-slate-300"
          >
            Pelanggan
          </h1>
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          >
            ...
            >
              ...
              >
              <span
                class="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap"
                >Template CSV</span
              >
            </button>
          ...
          >
            <span
              class="material-symbols-outlined text-[20px] group-hover:rotate-12 transition-transform duration-300"
              >person_add</span
            >
      ...
      <!-- Search & Filters -->
      <div
        class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700/60 p-4 animate-fade-in-up"
        style="animation-delay: 100ms"
      >
        <div class="relative group">
          <span
            class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors duration-300 text-[22px]"
            >search</span
          >
          <input
      ...
      >
        <div class="relative w-16 h-16">
          <div
            class="absolute inset-0 border-4 border-slate-200 dark:border-slate-700 rounded-full"
          ></div>
          <div
      ...
      <div
        ...
        class="flex flex-col items-center justify-center py-24 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-700 animate-scale-in"
      >
        <div
          class="w-24 h-24 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-6 shadow-inner"
        >
          <span class="material-symbols-outlined text-[48px] text-slate-400"
            >group_off</span
          >
        </div>
        ...
        <p
          class="text-slate-500 dark:text-slate-400 text-center max-w-md mb-8 leading-relaxed"
        >
          Database pelanggan Anda masih kosong. Mulai tambahkan pelanggan untuk
      ...
      >
        ...
        >
          ...
          <div
            ...
            :class="{ 'opacity-100': isSelected(customer.id) }"
          >
            <input
              type="checkbox"
          ...
          <div class="flex items-start gap-4 mb-6">
            <div
              class="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-100 to-blue-50 dark:from-blue-900/30 dark:to-blue-900/20 flex items-center justify-center text-blue-600 dark:text-blue-400 font-black text-2xl shadow-inner border border-white/50 dark:border-white/10 group-hover:scale-110 transition-transform duration-300"
            >
              {{ customer.name.charAt(0).toUpperCase() }}
            </div>
            <div class="flex-1 min-w-0 pt-1">
              <h3
                class="font-bold text-lg text-slate-900 dark:text-white truncate mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors"
              >
                {{ customer.name }}
              </h3>
              <div class="flex flex-col gap-0.5">
                <p
                  v-if="customer.email"
                  class="text-sm text-slate-500 dark:text-slate-400 truncate flex items-center gap-1.5"
                >
          ...
          <div
            class="grid grid-cols-2 gap-3 mb-6 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/50"
          >
            <div class="flex flex-col gap-1">
              <span
                class="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1"
              >
                <span class="material-symbols-outlined text-[14px]"
                  >shopping_bag</span
                >
            ...
            <div class="flex flex-col gap-1 text-right">
              <span
                class="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1 justify-end"
              >
                <span class="material-symbols-outlined text-[14px]"
                  >payments</span
                >
              ...
              <span
                class="font-black text-blue-600 dark:text-blue-400 text-lg"
          ...
          <div
            class="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700 flex items-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity"
          >
            <button
              v-show="
                canManageCustomers ||
      ...
      <div
        ...
        class="flex items-center justify-center gap-3 mt-8 pb-8 animate-fade-in"
      >
        <button
        ...
        <span
          class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full text-sm font-bold text-slate-600 dark:text-slate-300 shadow-sm"
        >
  ...
  >
    <div
      ...
      class="fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-4 bg-slate-900 dark:bg-slate-800 text-white px-6 py-4 rounded-2xl shadow-2xl border border-slate-700/50 backdrop-blur-md min-w-[320px] md:min-w-[400px]"
    >
      <div class="flex items-center gap-3 border-r border-slate-700 pr-4">
        <div
          class="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-xs font-black"
        >
          {{ selectedIds.length }}
        </div>
        <span class="text-sm font-bold whitespace-nowrap"
          >Pelanggan Terpilih</span
        >
      </div>
  ...
  >
    <div
      class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-6 animate-scale-in"
    >
      ...
      <div class="mb-4">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-500">
          ...
          <span class="font-bold text-amber-600">{{
            pointsCustomer?.loyaltyPoints || 0
          }}</span>
        </p>
      </div>
...
const handleAddPoints = async () => {
  if (!pointsCustomer.value || !pointsToAdd.value || pointsToAdd.value <= 0)
    return;

  try {
    addingPoints.value = true;

    await api.post(`/customers/${pointsCustomer.value.id}/loyalty-points`, {
      points: pointsToAdd.value,
    });

    await showSuccess(
      `Berhasil menambahkan ${pointsToAdd.value} poin ke ${pointsCustomer.value.name}`,
    );
...
const bulkDelete = async () => {
  ...
  try {
    loading.value = true;
    await Promise.all(
      selectedIds.value.map((id) => api.delete(`/customers/${id}`)),
    );
      ...
      (addons) => {
        try {
          ...
          return addons.some(
            (addon) =>
              addon &&
              addon.addonType === "DELIVERY_MARKETING" &&
              addon.status === "active",
          );
        } catch (error) {
...
const handleBulkMessage = async (data: {
  ...
  try {
    ...
    if (customersWithContact.length === 0) {
      await showError(
        `Tidak ada pelanggan terpilih yang memiliki ${data.type === "EMAIL" ? "email" : "nomor telepon"}`,
      );
      return;
    }
    ...
    if (data.type === "SMS" || data.type === "WHATSAPP") {
      ...
      await showSuccess(
        `Pesan ${data.type} berhasil dikirim ke ${customersWithContact.length} pelanggan`,
      );
    } else if (data.type === "EMAIL") {
      ...
      await showSuccess(
        `Pesan email berhasil dikirim ke ${customersWithContact.length} pelanggan`,
      );
    }
...
const handleExport = (format: "csv" | "excel" | "pdf" | "email") => {
  ...
  if (format === "csv") {
    exportToCSV(exportData, "daftar_pelanggan", [
      "Nama",
      "Email",
      "Telepon",
      "Alamat",
    ...
    exportToExcel(exportData, "daftar_pelanggan", [
      "Nama",
      "Email",
      "Telepon",
      "Alamat",
    ...
    exportToPDF(exportData, "daftar_pelanggan", "Daftar Pelanggan", [
      "Nama",
      "Email",
      "Telepon",
      "Alamat",
...
const handleEmailReport = async (title: string) => {
  ...
  try {
    await showSuccess(
      `Laporan "${title}" telah dijadwalkan untuk dikirim ke ${email}`,
    );
  } catch {
...
const handleFileImport = async (event: Event) => {
  ...
  reader.onload = async (e) => {
    ...
    if (customersToImport.length > 0) {
      const confirmed = await showConfirm(
        `Impor ${customersToImport.length} pelanggan?`,
      );
      if (confirmed) {
        try {
          loading.value = true;
          await Promise.all(
            customersToImport.map((c) => api.post("/customers", c)),
          );
          await showSuccess(
            `Berhasil mengimpor ${customersToImport.length} pelanggan`,
          );
