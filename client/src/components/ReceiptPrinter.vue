<template>
  <Teleport to="body">
    ...
    >
      <!-- Top Navigation -->
      <nav
        class="w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 shrink-0"
      >
          ...
          <div class="flex justify-between h-16">
            <div class="flex items-center gap-4">
              ...
              <div class="flex items-center gap-3">
                ...
                >
                  {{ tenantName || tenantInfo?.name || "Warungin POS" }}
                </h1>
              </div>
              <span
                class="h-6 w-px bg-slate-200 dark:bg-slate-700 mx-2 hidden sm:block"
              ></span>
            ...
            <div class="flex items-center gap-4">
              <div class="hidden md:flex flex-col items-end mr-2">
                <span
                  ...
                  >{{
                    receiptData?.cashierName || receiptData?.servedBy || "Kasir"
                  }}</span
                >
                <span class="text-xs text-slate-500">{{
                  receiptData?.shiftType
                    ? `Shift ${formatShiftType(receiptData.shiftType)}`
                    : "Kasir"
                }}</span>
              </div>
              <div
                class="size-9 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20"
              >
      ...
      <!-- Main Content Area -->
      <main
        class="flex-grow flex flex-col items-center justify-start py-6 px-4 sm:px-6 relative overflow-y-auto"
      >
        <!-- Background Pattern -->
        <div
          class="absolute inset-0 receipt-pattern opacity-40 pointer-events-none z-0"
        ></div>

        <!-- Action Toolbar -->
        <div
          class="w-full max-w-[420px] flex justify-between items-center mb-6 z-10"
        >
        ...
        >
          <div v-if="receiptData && template" class="receipt-content">
            <!-- Receipt Header -->
            <div
              class="flex flex-col items-center pt-8 pb-4 px-8 text-center border-b border-dashed border-slate-200"
            >
              <div
            ...
            <!-- Transaction Meta -->
            <div
              class="px-8 py-4 bg-slate-50 border-b border-dashed border-slate-200 flex flex-col gap-2"
            >
              <div
                v-if="template?.fields?.showOrderNumber"
                class="flex justify-between items-center text-xs font-medium text-slate-500"
              >
                <span>ID Transaksi</span>
                <span class="font-mono text-slate-900"
                  >#{{ receiptData.orderNumber }}</span
                >
              </div>
              <div
              ...
              >
                <span>Waktu</span>
                <span class="text-slate-900"
                  >{{ formatTime(receiptData.date) }} WIB</span
                >
              </div>
              <div
                v-if="
                  template?.fields?.showCashier &&
              ...
              <div
                v-if="
                  template?.fields?.showCustomer && receiptData.customerName
                "
                class="flex justify-between items-center text-xs font-medium text-slate-500"
              >
                <span>Pelanggan</span>
                ...
                >
                  <div class="flex flex-col flex-1">
                    ...
                    <span
                      ...
                      class="text-[10px] text-primary italic mt-0.5"
                    >
            ...
            <!-- Divider -->
            <div
              class="mx-6 border-t border-dashed border-slate-200 my-2"
            ></div>
            ...
            <div class="px-8 py-4 space-y-2">
              ...
              <div
                v-if="
                  template?.fields?.showDiscount && receiptData.discount > 0
                "
                class="flex justify-between text-sm text-slate-500"
              >
                <span>Diskon</span>
                <span class="font-medium text-green-600 tabular-nums"
                  >- {{ formatCurrency(receiptData.discount) }}</span
                >
              </div>
              <div
                v-if="template?.fields?.showTotal"
                class="pt-3 mt-2 border-t border-slate-200 flex justify-between items-end"
              >
            ...
            <div
              ...
              class="mx-8 mb-6 p-3 bg-primary/5 rounded-xl border border-primary/10 flex items-center justify-between"
            >
              <div class="flex items-center gap-2">
                ...
                <span
                  ...
                  >Lunas via
                  {{ getPaymentMethodLabel(receiptData.paymentMethod) }}</span
                >
              </div>
              <span
                v-if="receiptData.change && receiptData.change > 0"
                class="text-xs text-primary/70 font-mono"
              >
            ...
            <div
              v-if="
                template?.footer?.showThankYou || template?.footer?.showContact
              "
              class="bg-slate-50 border-t border-dashed border-slate-200 p-6 text-center"
            >
              <div class="flex flex-col items-center gap-2">
                <p
                  v-if="template?.footer?.showThankYou"
                  class="text-xs font-medium text-slate-900 italic"
                >
                ...
                <span
                  class="text-[10px] text-slate-500 font-mono mt-1 tracking-widest"
                  >{{ receiptData.orderNumber }}</span
                >
              </div>
            </div>
...
import {
  generateKlasikReceipt,
  generateModernReceipt,
} from "../utils/receipt-print-templates";
  ...
  try {
    ...
    if (Array.isArray(templates.value) && templates.value.length > 0) {
      const defaultTemplate =
        templates.value.find((t) => t && t.isDefault) || templates.value[0];
...
const loadReceiptData = async () => {
  ...
  try {
    const response = await api.get(
      `/receipts/generate/${props.orderId}?templateId=${selectedTemplate.value}`,
    );
    if (response.data) {
...
const printBrowser = async () => {
  ...
  if (!printWindow) {
    await warning(
      "Popup blocker terdeteksi. Silakan izinkan popup untuk mencetak struk.",
    );
    return;
  }
  ...
  const htmlContent =
    selectedPrintTemplate.value === "klasik"
