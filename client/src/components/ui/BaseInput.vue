<template>
  <div class="flex flex-col gap-1.5 font-display">
    ...
    <div class="relative group">
      ...
      <div
        v-if="icon"
        class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none transition-colors duration-300"
      >
        <span
      ...
      <div
        v-if="showSteppers && type === 'number' && !disabled"
        class="absolute inset-y-0 right-0 flex flex-col border-l border-slate-200 dark:border-slate-700 rounded-r-xl overflow-hidden"
      >
        <button
          type="button"
          @click="increment"
          :disabled="
            disabled || (max !== undefined && Number(modelValue) >= Number(max))
          "
        ...
        >
          <span
            class="material-symbols-outlined text-[16px] text-slate-600 dark:text-slate-300"
            >add</span
          >
        </button>
        <button
          ...
          @click="decrement"
          :disabled="
            disabled || (min !== undefined && Number(modelValue) <= Number(min))
          "
      ...
      <div
        v-if="$slots.append && (!showSteppers || type !== 'number')"
    ...
    <span
      v-if="error"
      class="text-xs font-bold text-red-500 ml-1 animate-fade-in-down flex items-center gap-1"
    >
