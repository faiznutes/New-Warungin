<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-scale-in"
        >
          <!-- Header -->
          <div
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div class="flex items-center justify-between mt-2">
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  <span v-if="messageType === 'SMS'">
                    ...
                    <span v-if="message.length > 160" class="text-amber-500"
                      >(akan terpotong menjadi beberapa SMS)</span
                    >
                  </span>
                  <span v-else> {{ message.length }}/1000 karakter </span>
                </p>
                <button
            ...
            >
              <div class="flex items-start gap-3">
                <span
                  class="material-symbols-outlined text-amber-600 dark:text-amber-400 text-[20px]"
                  >info</span
                >
                <div class="flex-1">
                  <p
                    class="text-sm font-bold text-amber-900 dark:text-amber-200 mb-1"
                  >
                    Peringatan
                  </p>
                  <p class="text-xs text-amber-800 dark:text-amber-300">
                    Pesan akan dikirim ke {{ selectedCount }} pelanggan.
          ...
          <!-- Footer -->
          <div
            class="flex items-center justify-between p-6 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50"
          >
            <button
              @click="close"
...
interface Emits {
  ...
  (
    e: "send",
    data: { message: string; type: "SMS" | "EMAIL" | "WHATSAPP" },
  ): void;
}
