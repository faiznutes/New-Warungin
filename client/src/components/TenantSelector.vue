<template>
  <div v-if="authStore.isSuperAdmin" class="mb-6">
    ...
    <div
      v-if="authStore.selectedTenantId && selectedTenantName"
    ...
    >
      ...
      <span
        >Menampilkan data:
        <strong class="text-gray-900">{{ selectedTenantName }}</strong></span
      >
    </div>
  </div>
</template>
...
const loadTenants = async () => {
  ...
  if (authStore.tenants.length > 0) {
    tenants.value = authStore.tenants.filter(
      (tenant: any) => tenant.name !== "System",
    );
    return;
  }
