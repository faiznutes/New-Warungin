      ...
      <div class="p-6">
        ...
        <div v-else-if="loading" class="flex items-center justify-center py-12">
          <div class="flex flex-col items-center">
            <div
              class="w-12 h-12 border-4 border-primary-600 border-t-transparent rounded-full animate-spin mb-4"
            ></div>
...
const loadSnapScript = (
  ...
  return new Promise((resolve, reject) => {
    ...
    if (existingScript) {
      console.log(
        "Snap.js script tag already exists, waiting for it to load...",
      );
  ...
  try {
    ...
    if (response.data.success && response.data.transactionId) {
      ...
      if (window.snap && snapToken.value) {
        console.log(
          "Embedding Snap.js with token:",
          snapToken.value.substring(0, 20) + "...",
        );
        ...
        error.value =
          "Gagal memuat halaman pembayaran. Snap.js tidak tersedia.";
        emit("error", error.value);
      }
    } else {
