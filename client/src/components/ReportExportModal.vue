<template>
  ...
  >
    <div
      class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in border border-white/20"
    >
      <div class="p-8">
        <div class="flex items-center justify-between mb-8">
          <div class="flex items-center gap-4">
            <div
              class="p-3 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-2xl"
            >
            ...
            <div>
              ...
              <p class="text-sm text-slate-500 font-medium">
                Unduh data laporan dalam format PDF atau CSV.
              </p>
            </div>
          </div>
          <button
        ...
        <form @submit.prevent="handleExport" class="space-y-6">
          ...
          <div>
            <label
              ...
              >Pilih Data yang Diekspor</label
            >
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              >
                <div
                  v-if="exportForm.exportType === 'report'"
                  class="absolute -right-3 -top-3 bg-blue-500 w-10 h-10 rounded-full flex items-end justify-start p-1.5"
                >
              ...
              <button
                ...
                class="px-4 py-4 rounded-2xl border-2 transition flex flex-col items-center gap-3 relative overflow-hidden group"
                :class="
                  exportForm.exportType === 'analytics'
                    ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400'
                    : 'border-slate-200 dark:border-slate-700 dark:bg-slate-800 hover:border-emerald-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                "
              >
              ...
              <button
                ...
                class="px-4 py-4 rounded-2xl border-2 transition flex flex-col items-center gap-3 relative overflow-hidden group"
                :class="
                  exportForm.exportType === 'both'
                    ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400'
                    : 'border-slate-200 dark:border-slate-700 dark:bg-slate-800 hover:border-emerald-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                "
              >
                <div
                  v-if="exportForm.exportType === 'both'"
                  class="absolute -right-3 -top-3 bg-blue-500 w-10 h-10 rounded-full flex items-end justify-start p-1.5"
                >
                  <span class="material-symbols-outlined text-white text-[14px]"
                    >check</span
          ...
          >
            ...
            <div class="relative">
              <span
                class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-slate-400 text-[20px]"
                >category</span
              >
              <select
              ...
              <span
                class="absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-slate-400 text-[20px] pointer-events-none"
                >expand_more</span
              >
            </div>
          </div>
          ...
          <div v-if="exportForm.format === 'PDF'" class="animate-fade-in">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
              ...
              <button
                ...
                class="px-2 py-3 rounded-xl border-2 transition flex flex-col items-center gap-1"
                :class="
                  exportForm.template === 'contemporary'
              ...
              <button
                ...
                class="px-2 py-3 rounded-xl border-2 transition flex flex-col items-center gap-1"
                :class="
                  exportForm.template === 'professional'
                    ? 'border-blue-600 bg-blue-600 text-white'
                    : 'border-slate-200 dark:border-slate-700 hover:border-blue-400 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                "
              >
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div class="text-center">
                  ...
                  <span
                    v-if="exportForm.exportType === 'analytics'"
                    class="block text-[10px] text-red-500 font-bold mt-1"
                    >Tidak tersedia untuk Analitik</span
                  >
                </div>
              </button>
              <button
              ...
              >
                <div
                  class="p-2 rounded-full"
                  :class="
                    exportForm.format === 'PDF' && canExportPDF
                ...
                <div class="text-center">
                  ...
                  <span
                    v-if="!canExportPDF"
                    class="block text-[10px] text-amber-600 dark:text-amber-400 font-bold mt-1"
                    >Hanya untuk paket berbayar</span
                  >
                </div>
              </button>
            </div>
          ...
          <div
            v-if="shareMode"
            class="pt-6 border-t border-slate-100 dark:border-slate-700 space-y-4"
          >
            <div
              class="bg-blue-50 dark:bg-blue-900/20 border border-emerald-200 dark:border-blue-800 rounded-xl p-4"
            >
              <div class="flex items-start gap-3">
                <span
                  class="material-symbols-outlined text-blue-600 text-[24px]"
                  >link</span
                >
                <div class="flex-1">
                  <h4
                  ...
                  <p class="text-xs text-slate-600 dark:text-slate-400 mb-3">
                    Salin link ini untuk dibagikan dengan tim atau stakeholder.
                  </p>
                  ...
                  <p
                    v-if="linkCopied"
                    class="text-xs text-blue-600 font-medium mt-2 flex items-center gap-1"
                  >
          ...
          <!-- Actions -->
          <div
            class="flex gap-4 pt-6 border-t border-slate-100 dark:border-slate-700"
          >
            <button
              type="button"
              @click="$emit('close')"
            ...
            >
              ...
              ></div>
              <span class="material-symbols-outlined text-[20px]" v-else
                >download</span
              >
...
const exportForm = ref({
  ...
  format: (authStore.isSuperAdmin || authStore.isSubscriptionActive
    ? "PDF"
    : "CSV") as "CSV" | "PDF",
  template: "contemporary" as
    | "clean"
    | "contemporary"
    | "vibrant"
    | "professional"
...
const canExportPDF = computed(
  () => authStore.isSuperAdmin || authStore.isSubscriptionActive,
);
...
const handleExport = async () => {
  ...
  try {
    ...
    if (
      (exportForm.value.exportType === "report" ||
        exportForm.value.exportType === "both") &&
      !reportData
    ) {
    ...
    if (
      (exportForm.value.exportType === "analytics" ||
        exportForm.value.exportType === "both") &&
      !analyticsData
    ) {
    ...
    if (exportForm.value.format === "CSV") {
      if (exportForm.value.exportType === "analytics") {
        await showWarning(
          "Export CSV hanya tersedia untuk laporan. Gunakan PDF untuk export analytics.",
        );
        return;
      }
      ...
      switch (exportForm.value.reportType) {
        ...
        case "product":
          exportData =
            reportData.products?.map((item: any) => ({
