        ...
        <div class="p-6">
          ...
          <div class="mb-4">
            <p class="text-sm text-gray-600 text-center mb-4">
              Pilih area gambar yang ingin di-crop. Gambar akan di-upscale
              menjadi 200x200 pixels.
            </p>
            <div class="flex items-center justify-center mb-4">
              <div
                class="w-32 h-32 border-2 border-dashed border-primary-500 rounded-xl flex items-center justify-center bg-gray-50"
              >
                <span class="text-xs text-gray-500 text-center px-2"
                  >Output: 200x200<br />(Auto Upscale)</span
                >
              </div>
            </div>
            <p class="text-xs text-gray-500 text-center">
              Gambar akan di-upscale menjadi 200x200 pixels, bahkan jika crop
              box lebih kecil
            </p>
          </div>
...
const handleDrag = (e: MouseEvent) => {
  ...
  newLeft = Math.max(
    0,
    Math.min(newLeft, imgRect.width - cropStart.value.width),
  );
  newTop = Math.max(
    0,
    Math.min(newTop, imgRect.height - cropStart.value.height),
  );
...
const handleResize = (e: MouseEvent) => {
  ...
  if (dir.includes("e")) {
    newWidth = Math.max(
      100,
      Math.min(
        cropStart.value.width + deltaX,
        imgRect.width - cropStart.value.left,
      ),
    );
  }
  if (dir.includes("w")) {
    const widthChange = Math.max(
      -cropStart.value.width + 100,
      Math.min(deltaX, cropStart.value.left),
    );
  ...
  if (dir.includes("s")) {
    newHeight = Math.max(
      100,
      Math.min(
        cropStart.value.height + deltaY,
        imgRect.height - cropStart.value.top,
      ),
    );
  }
  if (dir.includes("n")) {
    const heightChange = Math.max(
      -cropStart.value.height + 100,
      Math.min(deltaY, cropStart.value.top),
    );
