...
export class StoreShiftsService {
  ...
  async getCurrentShift(tenantId: string, outletId: string) {
  ...
  async getOpenShifts(tenantId: string, outletId?: string) {
    const where: any = {
      tenantId,
      status: "open",
    };
  ...
  async checkActiveShift(tenantId: string, outletId?: string) {
