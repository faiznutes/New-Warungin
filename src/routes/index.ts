...
// router.use('/analytics', analyticsRoutes); // Keep basic analytics? Audit said Disable marketing. Analytics might be useful. check plan. Plan says "advanced-*.routes.ts". General analytics might be OK. Wait. Plan says: "Disabled routes: marketing, retention, reward, accounting...". Analytics isn't explicitly prohibited but "email-analytics" is. I'll keep general analytics for now as it probably powers the dashboard.
// router.use('/finance', financeRoutes); // Finance might be needed for transactions? "No accounting" rule. But "transaction.routes" exists. Finance might be "accounting". Let's disable it if it's accounting.
...
// router.use('/payment-gateway', paymentGatewayIntegrationRoutes); // Maybe excessive? But plan listed it.
