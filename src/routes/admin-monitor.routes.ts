 ...
 */
router.get('/docker/containers', authGuard, roleGuard('SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.post('/docker/restart/:name', authGuard, roleGuard('SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.post('/docker/stop/:name', authGuard, roleGuard('SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { name } = req.params;
  // Escape container name for shell safety
  const escapedName = name.replace(/[^a-zA-Z0-9_-]/g, '');
  if (!escapedName || escapedName !== name) {
 ...
 */
router.get('/docker/logs/:name', authGuard, roleGuard('SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { name } = req.params;
  // Escape container name for shell safety
  const escapedName = name.replace(/[^a-zA-Z0-9_-]/g, '');
  if (!escapedName || escapedName !== name) {
 ...
 */
router.get('/server/resources', authGuard, roleGuard('SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.get('/health', authGuard, roleGuard('SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
  const services = [];
 ...
 */
router.get('/logs/:type', authGuard, roleGuard('SUPER_ADMIN'), asyncHandler(async (req: AuthRequest, res: Response) => {
