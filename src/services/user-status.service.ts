...
export async function updateUserStatusBasedOnSubscription(tenantId: string) {
  try {
    ...
    if (isExpired) {
      ...
      logger.info(`✅ Deactivated CASHIER, KITCHEN, SUPERVISOR users for tenant ${tenantId} due to expired subscription (ADMIN_TENANT excluded)`);
    } else {
      ...
      logger.info(`✅ Activated CASHIER, KITCHEN, SUPERVISOR users for tenant ${tenantId} due to active subscription`);
    }
    ...
    logger.error(`Error updating user status for tenant ${tenantId}:`, { error: error.message, stack: error.stack });
    throw error;
  }
}
...
export async function checkAndUpdateAllTenantsUserStatus() {
  try {
    ...
    logger.info(`✅ Updated user status for ${successful} tenants, ${failed} failed`);
    ...
    logger.error('Error checking and updating all tenants user status:', { error: error.message, stack: error.stack });
    throw error;
  }
}
